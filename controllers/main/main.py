...
from util import *
from your_controller_ekf_slam import CustomController
...
while driver.step() != -1:
    ...
    customController.update(timestep, driver)
    ...
    XVec.append(X)
    YVec.append(Y)
    deltaVec.append(-clamp(delta, np.radians(-30), np.radians(30)))
    xdotVec.append(xdot)
    ydotVec.append(ydot)
    psiVec.append(psi)
    psidotVec.append(psidot)
    FVec.append(clamp(F, 0, throttleConversion))
...
if finish:
    evaluation(minDist, trajectory, XVec, YVec, deltaVec)
